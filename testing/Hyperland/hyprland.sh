...
mount -t tmpfs -o size=3G,mode=755,noatime none /mnt
mkdir -p /mnt
mkdir -p /mnt/{nix,home,boot}
mkdir -p /mnt/home/user
mount -t tmpfs -o size=1G,mode=755,noatime,nosuid,nodev none /mnt/home/user
...
#pprint "Installing secureboot packages..." 
#sudo nix-env -iA nixos.sbctl

pprint "moving .nix package into /mnt/etc/nixos/'."

sudo cp *.nix /mnt/etc/nixos/
sudo cp -r homehyprland /mnt/etc/nixos/
sudo cp -r nixconf /mnt/etc/nixos/
sudo cp -r dot /mnt/etc/nixos/
...
pprint "nixos-install --root /mnt --flake /mnt/etc/nixos#user --impure"
